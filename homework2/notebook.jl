...
# ╔═╡ 435dce48-6e2e-4793-94fe-b63d5fc80812
using CairoMakie, DataFrames, Graphs, GraphMakie

# ╔═╡ f81d981e-4271-4448-bf4f-9b889a1714b6
begin
	using JSServe
	Page()
end

# ╔═╡ 7ddcf5e9-aa81-4fc8-9fac-2ac66e24d8c7
import CSV:read; df = read("Node-Location.csv", DataFrame)

# ╔═╡ d4dcf214-09e7-416e-9678-31b563421272
points = permutedims(Matrix(df[:, [3,4]]))

# ╔═╡ 425e651e-1c0a-40f9-92fe-1d4d0e012e59
g, w = euclidean_graph(points)

# ╔═╡ bc2f4e80-2773-4e29-b8a4-93201231a4a8
begin
	T = 0.1
	θ = 1
	for e in edges(g)
...
# ╔═╡ 404d349d-2ef5-42cd-baeb-2e8da2a9be5d


# ╔═╡ 4f3e6a5d-7239-4e4a-8957-c88a43179cce
myLayout = [Point(df[i, :Lat], df[i, :Lon]) for i in range(1,8)]

# ╔═╡ 73566502-3218-4875-a30f-1728e18884b3
_, _, p = graphplot(g, layout=myLayout;
					ilabels=df[:,:Name])
...
manifest_format = "2.0"
project_hash = "c25b8d6242419e4c6851bfc4bc8ff11090b838fd"
